...
const EMPTY_FILES = {
  fastq: null, germline_excel: null, somatic_excel: null,
  processed_germline: null, processed_somatic: null,
};
    ...
    <div>
      ...
      <label htmlFor={id}
        ...
        style={{
          ...
          justifyContent: "center", gap: 10, padding: "22px 16px", borderRadius: 12, cursor: "pointer",
          border: `2px dashed ${file ? color : "#e2e8f0"}`, background: file ? bgColor : "#fafbfc",
...
// ─── Step 1 — patient_id + sid both required, highlighted in amber box ────────
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: "#92400e", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Field label="Age" error={errors.age}>
          <TextInput value={form.age} onChange={v => set("age", v)} placeholder="e.g. 34" type="number" hasError={!!errors.age} />
        </Field>
        <Field label="Gender">
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Field label="Case Label">
          <TextInput value={form.diagnosis} onChange={v => set("diagnosis", v)} placeholder="e.g. CON" />
        </Field>
        <Field label="Disease Type">
          <SelectInput value={form.disease_type} onChange={v => set("disease_type", v)} placeholder="Select type"
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Field label="DNA Availability">
          <RadioGroup value={form.dna_availability} onChange={v => set("dna_availability", v)} options={["Yes", "No", "Pending"]} />
        </Field>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Field label="Sequencing">
          <SelectInput value={form.sequencing} onChange={v => set("sequencing", v)} placeholder="Select type"
            options={["WES", "WGS", "RNA-Seq", "Panel", "Other"]} />
        </Field>
        <Field label="Data Received">
          <RadioGroup value={form.data_received} onChange={v => set("data_received", v)} options={["Yes", "No"]} />
        </Field>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Field label="Data Analysed">
          <RadioGroup value={form.data_analysed} onChange={v => set("data_analysed", v)} options={["Yes", "No"]} />
        </Field>
      </div>
      <Field label="Sample Leveling">
        <SelectInput value={form.sample_leveling} onChange={v => set("sample_leveling", v)} placeholder="Select level"
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <div style={{ marginTop: 16 }}>
          <FileSlot label="FASTQ File" accept=".fastq,.fastq.gz,.fq,.fq.gz" file={files.fastq}
            onChange={f => setFile("fastq", f)} icon="🧬" color="#7c3aed" bgColor="#f5f3ff" borderColor="#ddd6fe" />
        </div>
      </div>
      <div>
        <Divider label="Analysis Excel Files" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
          <FileSlot label="Germline Excel" accept=".xlsx,.xls,.csv" file={files.germline_excel}
            onChange={f => setFile("germline_excel", f)} icon="📊" color="#16a34a" bgColor="#f0fdf4" borderColor="#bbf7d0" />
          <FileSlot label="Somatic Excel" accept=".xlsx,.xls,.csv" file={files.somatic_excel}
            onChange={f => setFile("somatic_excel", f)} icon="📈" color="#d97706" bgColor="#fffbeb" borderColor="#fde68a" />
        </div>
      </div>
      <div>
        <Divider label="Processed Output Files" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
          <FileSlot label="Processed Germline File" accept=".xlsx,.xls,.csv,.vcf,.txt" file={files.processed_germline}
            onChange={f => setFile("processed_germline", f)} icon="🧪" color="#2563eb" bgColor="#eff6ff" borderColor="#bfdbfe" />
          <FileSlot label="Processed Somatic File" accept=".xlsx,.xls,.csv,.vcf,.txt" file={files.processed_somatic}
            onChange={f => setFile("processed_somatic", f)} icon="🔭" color="#db2777" bgColor="#fdf2f8" borderColor="#fbcfe8" />
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "52px 28px 44px", textAlign: "center" }}>
      <div style={{
        ...
        background: "linear-gradient(135deg, #d1fae5, #a7f3d0)", border: "3px solid #6ee7b7",
        display: "flex", alignItems: "center", justifyContent: "center",
        marginBottom: 24, fontSize: 36, boxShadow: "0 0 0 10px rgba(16,185,129,0.08)",
      ...
      <p style={{ fontSize: 14, color: "#64748b", marginBottom: 28, lineHeight: 1.7, maxWidth: 340, animation: "slideUp 0.3s ease 0.15s both" }}>
        <strong style={{ color: "#0f172a" }}>{patient?.name || "The patient"}</strong> has been registered and is ready for further processing.
      </p>
      <div style={{
        ...
        {summaryFields.map(([label, val]) => (
          <div key={label}>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: "#1e293b", fontFamily: ["Patient ID","AOB ID","SID"].includes(label) ? "'DM Mono', monospace" : "inherit" }}>{String(val)}</div>
          </div>
        ))}
      </div>
      {uploadedFiles.length > 0 && (
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: "14px 20px", marginBottom: 32, width: "100%", maxWidth: 420, textAlign: "left", animation: "slideUp 0.3s ease 0.22s both" }}>
      ...
      <div style={{ display: "flex", gap: 12, width: "100%", maxWidth: 420, animation: "slideUp 0.3s ease 0.25s both" }}>
        <button onClick={onAddAnother} style={{ flex: 1, padding: "11px 16px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", color: "#475569", fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}
          onMouseEnter={e => { e.currentTarget.style.background = "#f8fafc"; e.currentTarget.style.borderColor = "#cbd5e1"; }}
          onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.borderColor = "#e2e8f0"; }}
        >+ Add Another</button>
        <button onClick={onGoToRecords} style={{ flex: 1, padding: "11px 16px", borderRadius: 10, border: "none", background: "#2563eb", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 2px 8px rgba(37,99,235,0.3)", transition: "all 0.15s" }}
...
export default function AddPatientPage() {
  ...
  // ── Validation: patient_id AND sid both required ──────────────────────────
  const validate = (s) => {
    const e = {};
    if (s === 1) {
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        patient_id: form.patient_id,              // ← now included in payload
        AOB_id: form.aob_id,
        sid: form.sid,
        name: form.name,
        age: form.age,
  ...
  const handleAddAnother = () => {
    setForm(EMPTY_FORM); setFiles(EMPTY_FILES); setErrors({});
    setStep(1); setSubmitted(false); setSavedPatient(null); setSubmitError(null);
  };
      ...
      <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e8edf3", padding: "0 32px", position: "sticky", top: 0, zIndex: 100 }}>
          <div style={{ maxWidth: 860, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 70 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 17, fontWeight: 700, color: "#0f172a", letterSpacing: "-0.01em" }}>Add New Patient</div>
                <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 500, marginTop: 1 }}>Exome Patient Management</div>
              </div>
            </div>
            <button onClick={() => navigate("/")} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 9, border: "1.5px solid #e2e8f0", background: "#fff", color: "#475569", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}
              onMouseEnter={e => { e.currentTarget.style.background = "#f8fafc"; e.currentTarget.style.borderColor = "#cbd5e1"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.borderColor = "#e2e8f0"; }}
            >
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "36px 32px 60px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e8edf3", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", overflow: "hidden" }}>
            {submitted ? (
              <SuccessScreen patient={savedPatient} files={files} onAddAnother={handleAddAnother} onGoToRecords={() => navigate("/")} />
            ) : (
              <>
                {/* Card header */}
                ...
                <div style={{ padding: "20px 28px", borderTop: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#fafbfc" }}>
                  <button onClick={step === 1 ? () => navigate("/") : back} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 20px", borderRadius: 9, border: "1.5px solid #e2e8f0", background: "#fff", color: "#475569", fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}
                  ...
                  {step < STEPS.length ? (
                    <button onClick={next} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 24px", borderRadius: 9, border: "none", background: "#2563eb", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", transition: "all 0.15s", boxShadow: "0 2px 8px rgba(37,99,235,0.3)" }}
                      onMouseEnter={e => { e.currentTarget.style.background = "#1d4ed8"; e.currentTarget.style.transform = "translateY(-1px)"; }}
                      onMouseLeave={e => { e.currentTarget.style.background = "#2563eb"; e.currentTarget.style.transform = "none"; }}
                    >
                      Continue
                    ...
                    <button onClick={submit} disabled={submitting} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 24px", borderRadius: 9, border: "none", background: submitting ? "#93c5fd" : "#16a34a", color: "#fff", fontSize: 14, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", transition: "all 0.15s", boxShadow: submitting ? "none" : "0 2px 8px rgba(22,163,74,0.35)" }}
                    ...
                    >
                      {submitting ? "Saving..." : (<><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>Save Patient</>)}
                    </button>
                  )}
                </div>
              </>

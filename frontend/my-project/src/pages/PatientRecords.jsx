...
export default function PatientRecords() {
  ...
  function handlePatientClick(patientId, e) {
    if (e.detail === 2) {
      ...
      navigate(`/view-patient/${selectedSample.patient?.id}/${selectedSample.sample?.id}`);
      return;
    }
                  ...
                  <thead>
                    <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e8edf3" }}>
                      ...
                      {["Patient ID", "Name", "Gender", "Samples", ""].map((h, i) => (
                        <th key={i} style={{
                          padding: "13px 16px", textAlign: i === 4 ? "right" : "left",
                ...
                {loadingDetail ? (
                  ...
                  <div style={{ animation: "fadeIn 0.2s ease", paddingTop: 16 }}>
                    ...
                    <div style={{ paddingTop: 16 }}>
                      <button
                        onClick={() => navigate(`/view-patient/${selectedSample.patient?.id}/${selectedSample.sample?.id}`)}
                        style={{

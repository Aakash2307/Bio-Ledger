...
function Avatar({ name }) {
  ...
  return (
    <div style={{
      ...
      fontSize: 24, fontWeight: 500, color: "#2563eb",
    }}>
      {initials}
    </div>
  );
...
const inputStyle = {
  ...
  border: "1.5px solid #e2e8f0", background: "#fafbfc", color: "#0f172a",
...
function EditSection({ icon, title, children, delay = 0 }) {
  return (
    <div style={{
      ...
      boxShadow: "0 2px 10px rgba(37,99,235,0.06)", overflow: "hidden",
      animation: `slideUp 0.3s ease ${delay}s both`,
    }}>
      <div style={{
        display: "flex", alignItems: "center", gap: 10,
        padding: "15px 24px", borderBottom: "1px solid #dbeafe",
...
export default function ViewPatientPage() {
  ...
  const [sample,   setSample]   = useState(null); // single sample object (first/primary)
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Use the first sample as the primary sample record
        const sampleData = (data.samples && data.samples.length > 0) ? data.samples[0] : {};

        setPatient(patientData);
        setSample(sampleData);
        // Merge both into form so edits can touch either
  ...
  const handleCancel = () => {
    setForm({ ...patient, ...sample }); // reset to original
    setEditing(false);
  };
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 32px 60px" }}>
              ...
              {editing ? (
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(175px, 1fr))", gap: "16px 28px" }}>
                    ...
                    <EditRadio label="Patient Status" value={form?.patient_status}
                      onChange={v => set("patient_status", v)}
                      options={["New", "In Progress", "Completed", "Active", "Inactive"]} wide />
                  </div>
                </div>
              ) : (
                      ...
                      {[
                        ...
                        <div key={f.label} style={{
                          ...
                          <span style={{
                            ...
                            color: "#1e293b", fontFamily: "'DM Mono', monospace",
                          }}>{f.value}</span>
                        </div>
                      ))}
                      {patient.age && (
          ...
          {!loading && patient && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              {/* ── SECTION 1: Clinical Information — from patients table ── */}
              {editing ? (
                ...
                <Section icon="🩺" title="Clinical Information" delay={0.05}>
                  <FieldGrid>
                    {/* all from patients table */}
                    <FieldItem label="Detail Disease"  value={patient.detail_disease} wide />
                    <FieldItem label="Organ Type"      value={patient.organ_type} />
                    <FieldItem label="Comorbidity"     value={patient.comorbidity} />
                    <FieldItem label="Family History"  value={patient.family_history} />
                    <FieldItem label="Metastasis"      value={patient.metastasis} />
                    <FieldItem label="Consultation"    value={patient.consultation} wide />
                  </FieldGrid>
                </Section>
              )}

              {/* ── SECTION 2: Sample Information — from samples table ── */}
              {editing ? (
              ...
              {/* ── SECTION 3: Sequencing — from samples table ── */}
              {editing ? (
              ...
              {/* ── SECTION 4: Data & Analysis — from samples table ── */}
              {editing ? (
              ...
              {/* ── SECTION 5: Report — from samples table ── */}
              {editing ? (
